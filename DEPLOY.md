...
2. **生产库还没建表**：Postgres 在生产不会自动 `push`。确认 Build Command 是 `pnpm run ci`（先跑 `scripts/force-migrate.ts` 再 build）。若库里曾有 `payload_migrations.batch = -1`（dev push 痕迹），Vercel 的 `CI=true` 会让 migrate 确认框默认「否」并静默退出，新表（如 `joys`）建不上、Admin 一片空白；`force-migrate` 会先删掉这类记录再迁移。缺 migration 时本地执行 `pnpm payload migrate:create` 后重新部署。
